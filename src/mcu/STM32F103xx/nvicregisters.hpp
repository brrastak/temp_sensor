...
struct NVIC
{
  ...
  {
    using INTLINESNUM = NVIC_ICTR_INTLINESNUM_Values<NVIC::ICTR, 0, 4, ReadMode, NVICICTRBase> ;
    using FieldValues = NVIC_ICTR_INTLINESNUM_Values<NVIC::ICTR, 0, 0, NoAccess, NoAccess> ;
  } ;

  template<typename... T> 
  using ICTRPack  = Register<0xE000E004, 32, ReadMode, NVICICTRBase, T...> ;

  struct NVICSTIRBase {} ;

  struct STIR : public RegisterBase<0xE000EF00, 32, WriteMode>
  {
    using INTID = NVIC_STIR_INTID_Values<NVIC::STIR, 0, 9, WriteMode, NVICSTIRBase> ;
    using FieldValues = NVIC_STIR_INTID_Values<NVIC::STIR, 0, 0, NoAccess, NoAccess> ;
  } ;

  template<typename... T> 
  using STIRPack  = Register<0xE000EF00, 32, WriteMode, NVICSTIRBase, T...> ;
