...
struct RegisterField
{
        ...
        template<typename T = AccessMode, class = typename std::enable_if_t<std::is_base_of<ReadWriteMode, T>::value>>
		inline static void Set(RegType value)
		{
		...
		//Метод устанавливает значение битового поля, только в случае, если оно достпуно для записи
		template<RegType value, typename T = AccessMode, class = typename std::enable_if_t<std::is_base_of<ReadWriteMode, T>::value>>
		...
		//Метод устанавливает значение битового поля, только в случае, если оно достпуно для записи
		template<typename T = AccessMode, class = typename std::enable_if_t<std::is_base_of<WriteMode, T>::value>>
		inline static void Write(RegType value)
		{
				// assert((size < sizeof(RegType) * 8U) ? (value <= ((static_cast<RegType>(1U) << size) - static_cast<RegType>(1U))) :
				// 			 (value <= std::numeric_limits<RegType>::max()));
		...
		//Метод устанавливает значение битового поля, только в случае, если оно достпуно для записи
		template<RegType value, typename T = AccessMode, class = typename std::enable_if_t<std::is_base_of<WriteMode, T>::value>>
		...
		template<typename T = AccessMode, class = typename std::enable_if_t<
